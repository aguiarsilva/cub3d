...
void	ft_initialize_raycasting_data(int x, t_ray *ray, t_player *player)
{
	...
	ray->ray_distance.ray_dir_x = player->dir_x + player->plane_x * ray->ray_distance.camera_x;
	ray->ray_distance.ray_dir_y = player->dir_y + player->plane_y * ray->ray_distance.camera_x;
...
void	ft_set_dda(t_ray *ray, t_player *player)
{
	...
	{
		ray->step_x = -1;
		ray->ray_distance.side_dist_x = (player->x_pos - ray->map_x) * ray->ray_distance.delta_dist_x;
	}
	else
	{
		ray->step_x = 1;
		ray->ray_distance.side_dist_x = (ray->map_x + 1.0 - player->x_pos) * ray->ray_distance.delta_dist_x;
	}
	if (ray->ray_distance.ray_dir_y < 0)
	{
		ray->step_y = -1;
		ray->ray_distance.side_dist_y = (player->y_pos - ray->map_y) * ray->ray_distance.delta_dist_y;
	}
	else
	{
		ray->step_y = 1;
		ray->ray_distance.side_dist_y = (ray->map_y + 1.0 - player->y_pos) * ray->ray_distance.delta_dist_y;
	}
}
...
void	ft_calculate_line_height(t_ray *ray, t_game_data *game_data, t_player *player)
{
	if (ray->side == 0)
		ray->ray_distance.wall_dist = (ray->ray_distance.side_dist_x - ray->ray_distance.delta_dist_x);
	else
		ray->ray_distance.wall_dist = (ray->ray_distance.side_dist_y - ray->ray_distance.delta_dist_y);
	ray->line_height = (int)(game_data->win_height / ray->ray_distance.wall_dist);
	...
	if (ray->side == 0)
		ray->ray_distance.wall_x = player->y_pos + ray->ray_distance.wall_dist * ray->ray_distance.ray_dir_y;
	else
		ray->ray_distance.wall_x = player->x_pos + ray->ray_distance.wall_dist * ray->ray_distance.ray_dir_x;
